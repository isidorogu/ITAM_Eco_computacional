---
title: 'Lec10: Inferencia Causal: Experimentos I'
...
El sesgo de selección significa que los grupos que recibieron el tratamiento y los que no (el grupo control) **son diferentes**. 

- Esto hace que la comparativa entre los grupos sea injusta. No podemos atribuirle las diferencias al tratamiento porque los grupos ya eran distintos.

- En un mundo ideal, te gustaría que la **única** diferencia entre los grupos fuera el tratamiento.

Hay dos tipos de sesgo de selección (en pre-intervención!): 
...
En ambos casos de selección, la implicación es que la media observada para cada grupo no es independiente a el grupo donde esta el individuo ya que este "se seleccionó" para el grupo: 
...
Estamos ante la búsqueda de una asignación de tratamiento que ayude a hacer inferencia causal
...
2. **Asignación no aleatorio (métodos cuasi experimentales)**: Buscamos, entendiendo la función de asignación que no controlamos, estimar el contrafactual (conditional unconfoundness).
...
Experimento: Conjuntos de grupos de tratamiento y probabilidades de pertenecer a cada uno. 
...
5. **Diseños clusterizados**: El nivel de aleatorización cambia
...
### Con que variables estratificamos? 

Debemos elegir las variables sobre las cuales estratificar con base en dos criteros: 

1. Variables que creemos están muy **correlacionadas con el impacto de tratamiento $\tau$**

2. Variables sobre las que tengamos interés en conocer efectos heterógeneos. 


### Mecánica de variables estratificadoras

La idea es construir bloques con **variables categóricas**. Con ellas se crean los estratos a partir de las combinaciones de cada categoria.
...
Si las variables que nos interesa estratificar son continuas, debemos categorizarlas de alguna manera (i.e. deciles, cuartiles, etc).


### Impacto de la estratificación sobre la medición de impacto 

En general, si elegimos variables que están correlacionadas con el impacto de tratamiento, vamos a ganar mucha eficiencia en los estimadores (menos varianza)

Si elegimos variables no correlacionadas, no hay daño. Simplemente el estimador se parece más al estimador de un experimento de aleatorización simple

\begin{center}
\includegraphics[width = 8cm]{block_impact.png}
\end{center}
...
- Debemos asignar los misfits de 3 maneras (global, strata, NA)
...
$$Pr(|T| > \Phi^{-1}(1-\frac{\alpha}{2})) = \Phi \left( -\Phi^{-1}(1-\frac{\alpha}{2}) + \frac{\tau}{\sqrt{ \sigma^2(\frac{1}{N_T}+\frac{1}{N_c})}}\right)$$
...
- Pruebas conjuntas F: Correr un modelo de probabilidad lineal de pertenecer a cada tratamiento vs el grupo control. 

### Balance tests: F-test

Cuando el número de variables a evaluar son grandes, entramos en un problema de multiple hypothesis testing (i.e. Bonferroni adjustments). 
...
- La inferencia causal se trata sobre la **función de asignación**. Para que esta nos ayude a arrojar estimadores causales necesitamos que cumpla 3 características: 

1. Asignación individual (No contaminación): Stable Unit Treatment Variable Assignment 

2. Asignación Probabilística (Overlap): No hay selección en variables

3. Unconfoundness: El tratamiento es independiente a las variables

Los experimentos son el Gold standard de la inferencia causal: 

- La asignación aleatoria permite cumplir condiciones 2 y 3. 

- El Data Scientist tiene que estar pendiente de si se cumple 1

- La asignación estratificada es mejor que la asignación simple, pues te permite aumentar la eficiencia de los estimadores y observar efecto heterogeneos. 
