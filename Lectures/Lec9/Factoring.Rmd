...
Tenemos una base de datos con $n$ filas y $p$ columnas. $X_{n,p}$

Factores: 

$$F=X\phi$$

Los loadings: cuánto importa cada variable en el factor 

$$\phi \in \Re^p$$


Los factores son 1 variable 

$$F \in \Re^n$$

Cuántos factores? Depende! No necesita ser 1. Necesita ser menos que la cantidad de columnas $p$
...
### PCA como Eigenvectores 

Recordemos que son los eigenvectores y el eigenvalor: 

Tenemos una matriz $A_{\{m,m \}}$ 

Tenemos que si multiplicamos la matriz $A$ por un **eigenvector** $v$, nos resulta el mismo vector $v$ multiplicado por una **eigenvalor** $\lambda$

$$Av=\lambda v$$

Esto es genial! Existe un vector eigen que, multiplicado por cualquier matriz, reduce su dimensión en gran medida. 

$$(A-\lambda)v=0$$

Como lo podríamos aplicar a análisis de factores? Noten como A es una matriz de dimensiones simétricas

Finalmente, noten como una matriz A de 4x4 tiene exactamente 4 eigenvectores!!! 

### PCA como Eigenvectores

Partamos de la base $X_{n,p}$. 

Si calculamos la matriz de varianza covarianza $\Sigma$


$$ \Sigma= 
\left[\begin{array}{cc} var(x_1) & cov(x_1,x_2) \\ cov(x_1, x_2) & var(x_2) \end{array}\right]$$ 
...
La matriz de varianza covarianza es una forma de ver la dispersión (varianza) y orientación (covarianza) de los datos.

\begin{center}
\includegraphics[width = 10cm]{sigma_matrix.png}
\end{center}


### PCA como Eigenvectores

La dirección del vector es en donde hay mas dispersión. La magnitud (eigenvalor) marca el tamaño del vector. El PCA es una manera de hacer eigenvectores e eigenvalores  


\begin{center}
\includegraphics[width = 7cm]{eigen.png}
\end{center}
...
$$d_k^2=\frac{1}{n}\Sigma_{i=1}^nz_{ik}^2$$

La proporción de varianza explicada (PEV): 

$$PEV=\frac{\Sigma_{j=1}^pVar(X_j)}{d_k^2}$$
...
# PCA: Will allow nice stratification to build representative samples of buy_currency_i == 0
################################
# Build X matrix
X<-
  universo_training %>% 
  ungroup() %>% 
  select(referred:days_to_trade, trades_btc_1:register_web)

X<-sparse.model.matrix(~. + 0, data = X)

pca<-prcomp(X, scale = T)

save(pca, file = 'PCA_undersampling.Rdata')
...
load('PCA_undersampling.Rdata')

# How many principal components. Up to 1% of explained additional variance.
# 22 components. 51.1% of explained variance 
...
pca_scores<-as_tibble(predict(pca)[, 1:10])
dim(pca_scores)
```


### PCA en `R`
```{r echo=TRUE, warning=FALSE, message=FALSE, fig.height=2, fig.width=3}
row.names(sum_pca)<-c("std_k", "share_variance", "cum_share")
sum_pca1<-t(sum_pca)

sum_pca1<-as_tibble(sum_pca1)
sum_pca1$component<-seq(1:246)

ggplot(sum_pca1, aes(component,cum_share))+
...
```{r echo=TRUE, warning=FALSE, message=FALSE, fig.height=2, fig.width=3}
ggplot(sum_pca1, aes(component,std_k))+
  geom_point()+geom_path()+theme_bw()
```
...
$$x_{ij}=\Sigma_{k=1}^Kz_{ik}\phi_{jk}$$
...
$$E[x_{ik}]=\phi_{j1} z_{i1}+...+\phi_{jK} z_{iK}$$
