...
### Ejemplo Paramétrico: Regresión Lineal

La regresión es lineal en parámetros. No obstante, podemos estimar funciones polinómicas no lineales sobre las $X$

\begin{center}
\includegraphics[width = 8cm]{parametrica.png}
\end{center}
...
### Ejemplo No paramétrico: K Nearest Neighbors

La línea negra es la relación verdadera. Noten como con $k=1$ la función que se estima es super compleja. Con $k=9$ es una función más suave y parecida a la relación verdadera.
...
1) Mínimos Cuadrados Ordinarios ($min_{\{\beta \}}\Sigma_{i=1}^N(y_i - \hat{y})^2$) o 
...
$$\hat{\beta_0}=\bar{y}-x_k\hat\beta_k$$
...
Donde $\hat y_i = \beta_0 + x_k\hat \beta_k$ son los valores estimados y $\hat u_i$ son los **residuales**. 

La suma de los residuales es siempre cero: 

$$\Sigma_i^n\hat u_i = 0$$

La covarianza de las varianbles $x_k$ y los residuales $\hat u_i$ es siempre cero:

$$\Sigma_i^n\hat u_ix_i = 0$$
...
Noten como usé la propiedad  $\Sigma_i^nx_iu_i=0$. 

$$\hat \beta \sim N(\beta, (X'X)^{-1}\sigma^2)$$

### Varianza del estimador (Error estándar)

Veamos de cerca la varianza de los estimadores: 

$$Var(\beta)=(X'X)^{-1}\sigma^2=\frac{\sigma^2}{\Sigma_i^n(x_{ik}-\bar{x_k})^2(1-R_a^2)}$$

Donde $R_a^2$ es la bondad de ajuste de correr la regresión $x_k \sim x_{-k}$. 

Es decir, la regresión de la variable x contra todas las otras. Noten como, mientras mas correlacionadas están las otras variables a $x_k$, más varianza se incluye. Queremos variables que sean lo más independientes posibles! Esto se ve matricialmente en $X'X$



### Varianza del estimador (Error estándar) II

Problema: No podemos saber si un estimador es significativo porque su error estándar depende de un parámetro ($\sigma^2 = Var(y|x)=Var(u_i)$). La varianza del **error**.

Podemos estimarla! Usamos los **residuales** para ello $\hat u_i$

$$\hat \sigma^2=\frac{1}{n-2}\Sigma_1^n \hat {u_i}$$

Con esto, podemos estimar los errores estándar de los estimadores $\beta$, $se(\beta)=\frac{\hat \sigma}{(\Sigma_1^n(x_{ik}-\bar x_k)^2)^{1/2}}$
...
$$\hat{\beta}_k \rightarrow^p = \frac{Cov(x_i,y_i)}{Var(x_i)}=\beta$$ 

$$\hat \sigma^2 \rightarrow^p \sigma^2$$
...
$$RSS = \Sigma_{i=1}^n(y_i - \hat y_i)^2 = \Sigma_{i=1}^n\hat{u_i}^2 = n*ECM$$
