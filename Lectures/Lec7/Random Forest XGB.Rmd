...
\begin{center}
\includegraphics[width = 9cm]{adaboost.png}
\end{center}
...
- Típicamente $\gamma$ será el promedio de las observaciones para regresión o la moda para clasificación. $f(.)$ normalmente será un árbol; pero puede ser otro estimador $m \in \{1,m \}$
...
c) En cada región estima $\gamma_{jm}$: El promedio de las observaciones $r_{im}$ (o moda en clasificación)
...
- $\lambda$ learning rate: Este controla (regulariza) al ritmo al que el algoritmo lee. Baja el peso de cada nuevo árbol. Se interpreta como "escepticismo". Típicamente tiene valores de 0.01, 0.001.
...
Tomemos un ejemplo de [Towards Data Science](https://towardsdatascience.com/all-you-need-to-know-about-gradient-boosting-algorithm-part-1-regression-2520a34a502) para clarificar GBM. Imaginen que tenemos una base de datos con sólo $y$ y 1 $x$. Queremos construir un modelo predictivo de $\hat y=f(x)$.


\begin{center}
\includegraphics[width = 6cm]{y_x.png}
\end{center}

Ahora, el paso 1 define un $f_0 = \bar y$. Empezamos con un predictor sencillo! Noten como, formalmente, esto es $f_0(x_i) = argmin_{\gamma}\Sigma_{i=1}^NL(y_i, \gamma)$
...
\begin{center}
\includegraphics[width = 6cm]{r_im.png}
\end{center}
...
Con el árbol, promediamos los residuales ($\gamma_{jm}$) en cada nodo terminal $R_{jm}$. Hacemos que el modelo se enfoque en los errores!  

\begin{center}
\includegraphics[width = 10cm]{him.png}
\end{center}
...
Actualiza el modelo $f_m(x)=f_{m-1}(x)+\lambda\Sigma_{j=1}^{jm}\gamma_{jm}I(x \in R_{jm})$. En este ejemplo esto es:

$$f_1(x)=f_0+\lambda\gamma_{jm}I(x \in R_{jm})$$

$$f_1(x) =\Biggl\{ \ \ {f_0 + \lambda \cdot 6.0 \ \ \ if \ \ x<=49.5 \atop f_0 - \lambda \cdot 5.9 \ \ \ if \ \ x>4.5}$$
...
\begin{center}
\includegraphics[width = 6cm]{r2.png}
\end{center}

Estimo un árbol sobre $r_{i2}$: 

\begin{center}
\includegraphics[width = 6cm]{hi2.png}
\end{center}

### Ejemplo 

Actualizo el modelo, generando $f_2(x)=f_{1}(x)+\lambda\Sigma_{j=1}^{jm}\gamma_{j2}I(x \in R_{j2})$


$$f_2(x) =\Biggl\{ \ \ {f_1 - \lambda \cdot 10.1 \ \ \ if \ \ x<=4.5 \atop f_1 + \lambda \cdot 0.5 \ \ \ if \ \ x>4.5}$$
...
\begin{center}
\includegraphics[width = 10cm]{gbm1.png}
\end{center}
...
\begin{center}
\includegraphics[width = 8cm]{gbm2_4.png}
\end{center}
...
\begin{center}
\includegraphics[width = 10cm]{gbm5_6.png}
\end{center}

### Gamma y Lambda

$\lambda$ como dijimos, este valor añade algo de escepticimos $\lambda \in (0,1)$. Si es muy grande, el algoritmo va a terminar muy rápido (bajo $M$) y puede seguir ruido. Típicamente tiene valores pequeños (0.01, 0.001)

Veamos como "derivar" las $\gamma's$

$$f_0(x_i) = argmin_{\gamma}\Sigma_{i=1}^NL(y_i, \gamma)$$

$$f_0(x_i) = argmin_{\gamma}\Sigma_{i=1}^N0.5(y_i-\gamma)^2$$
CPO: $\partial L/ \partial \gamma$

$$\Sigma_{i=1}^N(y_i - \gamma^*)=0$$
$$\Sigma_{i=1}^Ny_i = N\gamma^*$$
$$\gamma^*=\frac{\Sigma_{i=1}^Ny_i}{N}$$

### Gamma 

$$\gamma_{jm} = argmin_{\gamma}\Sigma_{x_i \in R_{jm}}L(y_i, f_{m-1}(x_i)+\gamma)$$

$$\gamma_{jm} = argmin_{\gamma}\Sigma_{x_i \in R_{jm}}0.5(y_i - f_{m-1}(x_i)-\gamma)^2$$

CPO:

$$\Sigma_{i=1}^N(y_i -f_{m-1}(x_i)- \gamma^*)=0$$
$$N\gamma^*=\Sigma_{x_i \in R_{jm}}(y_i -f_{m-1}(x_i))$$
$$\gamma^*_{jm}=\frac{\Sigma_{x_i \in R_{jm}} r_{im}}{n_j}$$
...
La mejor libreria para correr GBMs es `gbm`


\begin{center}
\includegraphics[width = 10cm]{gbm_r.png}
\end{center}
...
\begin{center}
\includegraphics[width = 10cm]{gbm_r2.png}
\end{center}
...
```{r, message=F, warning=F, echo=T, fig.width=4.5, fig.height=3}
...
Los Extrem Gradient Boosting Machines son muy parecidos a los GBM; le agregan algunas cosas: 
...
- `lambda`: Regularización de los nodos terminales de cada arbol estilo LASSO 
