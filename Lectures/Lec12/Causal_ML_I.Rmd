...
Los estimadores doble robustos son muy parecidos a los estimadores IPW, pero ajustan por $\bar Y, \bar{e(X_i)}$. 
...
$$\tau_{AIPW}=\frac{1}{N}=\Sigma_{i=1}^N \Big(\hat{IPW} - \bar{IPW}*[\hat{e(X_i)}*\hat Y_{|T_i=0}-(1-\hat{e(X_i)})*\hat{Y}_{|T_i=1}] \Big)$$
